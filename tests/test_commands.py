...
def test_get_rule(runner, login_mock, mock_get_rules, print_table_mock):
    runner.invoke(
        ctl.get_rule,
        ["1", "2"],
    )
    assert mock_get_rules.call_args_list == [
...
def test_create_rule(runner, login_mock, mock_create_rules):
    runner.invoke(
        ctl.create_rule,
        ["1", "tag1", "tag2"],
    )
...
def test_delete_rule(runner, login_mock, mock_delete_rules):
    runner.invoke(
        ctl.delete_rule,
        ["1", "tag1", "tag2"],
    )
    mock_delete_rules.assert_called_once_with(
        mock.ANY,
        {
            "agent_id": 1,
            "tag_names": ("tag1", "tag2"),
